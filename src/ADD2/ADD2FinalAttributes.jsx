...
export default class ADD2FinalAttributes extends Component {
  constructor(props) {
    ...
    this.state = {
      height: 0, weight: 0, age: 0, hp: 0, funds: 0, moveRate: 0,
  ...
  async handleHPGP() {
    ...
    if (!moveRate) {
      ...
      this.setState({
        moveRate: data[0], paralyze: data[1], rod: data[2],
  ...
  handleUpdate() {
    const {
      height, weight, age, funds, hp, moveRate,
      paralyze, rod, petrification, breath, spell
    } = this.state;
