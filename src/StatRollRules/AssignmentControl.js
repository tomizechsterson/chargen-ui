    ...
    if(rolls.length === 1) {
        ...
        return (
            <div data-cy={'doubleAssignmentSelector' + stat}>
                <input type='button' onClick={() => onSelectStat(stat)} value={stat} /> {charStat}
                {rolls[0] && <input data-cy={'roll' + rolls[0].id} type='button' onClick={() => onSelectRoll(rolls[0])} value={rolls[0].value} disabled={rolls[0].assigned} />}
                {rolls[0] && <span style={rollStyle}>{rolls[0].text}</span>}
                {rolls[1] && <input data-cy={'roll' + rolls[1].id} type='button' onClick={() => onSelectRoll(rolls[1])} value={rolls[1].value} disabled={rolls[1].assigned} />}
