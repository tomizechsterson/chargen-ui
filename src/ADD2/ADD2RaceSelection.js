...
export default class ADD2RaceSelection extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <StatAdjustmentDisplay
          text={ 'STR: ' }
          stat={ selectedChar.str }
          adjustment={ adjustments && this.getAdjustment('str') }
        />
        <StatAdjustmentDisplay
          text={ 'DEX: ' }
          stat={ selectedChar.dex }
          adjustment={ adjustments && this.getAdjustment('dex') }
        />
        <StatAdjustmentDisplay
          text={ 'CON: ' }
          stat={ selectedChar.con }
          adjustment={ adjustments && this.getAdjustment('con') }
        />
        <StatAdjustmentDisplay
          text={ 'INT: ' }
          stat={ selectedChar.int }
          adjustment={ adjustments && this.getAdjustment('int') }
        />
        <StatAdjustmentDisplay
          text={ 'WIS: ' }
          stat={ selectedChar.wis }
          adjustment={ adjustments && this.getAdjustment('wis') }
        />
        <StatAdjustmentDisplay
          text={ 'CHR: ' }
          stat={ selectedChar.chr }
          adjustment={ adjustments && this.getAdjustment('chr') }
        />
        Select Race:
